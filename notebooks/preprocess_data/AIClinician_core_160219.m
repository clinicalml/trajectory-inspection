...
% This is the saved workspace at the end of AIClinician_mimic...
D = load(join([filepath, '/mimic-table.mat'], ''));
MIMICtable = D.MIMICtable;
...
% NOTE: Updated so that this only considers column 24... column 10-14 is eICU, not available in our setup

% if recqvi(modl,24)>0 & recqvi(modl,14)>0   % if 95% LB is >0 : save the model (otherwise it's pointless)
if recqvi(modl, 24) > 40  % This is an implicit condition of the previous if statement
